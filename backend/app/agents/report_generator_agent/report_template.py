...
class PharmReportTemplate:
    ...
    def _generate_css(self) -> str:
        ...
        :root {
            /* Core Brand - Light Professional Pharma */
            --primary: #0369a1;
            ...
            /* Agent Signature Colors - Deep for Light Mode */
            --iqvia: #0369a1;
            --clinical: #059669;
            --patent: #d97706;
            --exim: #0891b2;
            ...
            /* Surface System - Light Clean Professional */
            --background: #ffffff;
            ...
            --border-glow: rgba(3, 105, 161, 0.2);
            ...
            /* Shadows & Glows - Subtle Professional */
            --shadow-sm: 0 2px 8px rgba(0,0,0,0.08);
            --shadow-md: 0 8px 24px rgba(0,0,0,0.12);
            --shadow-lg: 0 20px 60px rgba(0,0,0,0.15);
            --shadow-glow: 0 0 60px rgba(3, 105, 161, 0.08);
            --shadow-accent: 0 8px 32px rgba(3, 105, 161, 0.15);
            --shadow-inner: inset 0 1px 0 rgba(0,0,0,0.05);
        }
        ...
        * {
        ...
        @media print {
            * {
            ...
            :root {
                --background: #ffffff !important;
                --background-elevated: #f9fafb !important;
                --surface: #f3f4f6 !important;
                --surface-light: #e5e7eb !important;
            ...
            /* Executive summary: keep on one page */
            .executive-summary {
                padding: 48px 48px;
            }
        ...
        /* ============================================
           PDF ORPHAN / WIDOW PREVENTION
           Keep cards, banners, headings+content together
           ============================================ */
        
        /* Cards and self-contained blocks: never split across pages */
        .recommendation-card,
        .metric-card,
        .summary-banner,
        .content-card,
        ...
        /* Section headers must stick with the content that follows */
        .section-header {
            break-inside: avoid;
            page-break-inside: avoid;
            break-after: avoid;
        ...
        /* Metrics grid rows: keep the whole grid together */
        .metrics-grid {
            break-inside: avoid;
            page-break-inside: avoid;
        }
        
        /* The executive summary sub-blocks */
        .executive-summary .key-takeaways,
        .executive-summary .recommendation-card,
        .executive-summary .opportunity-score-card {
            break-inside: avoid;
            page-break-inside: avoid;
        }
        
        /* Sections CAN break (they're too tall) but prefer not to */
        .section {
            break-before: auto;
        ...
        @media print {
            ...
            /* Sections are allowed to break — they're tall.
               Individual elements inside them are protected
               by the global break-inside rules above. */
            .section {
                break-inside: auto;
            }
        }
        ...
        /* ============================================
           CHART CONTAINERS - SVG Visualization
           ============================================ */
        .chart-container {
            background: var(--surface);
            border: 1px solid var(--border);
        ...
        .agent-status-card.iqvia::before { background: var(--iqvia); }
        .agent-status-card.clinical::before { background: var(--clinical); }
        .agent-status-card.patent::before { background: var(--patent); }
        .agent-status-card.exim::before { background: var(--exim); }
        .agent-status-card.internal::before { background: var(--internal); }
        .agent-status-card.web::before { background: var(--web); }
    ...
    def _render_table_of_contents(self, data: Dict) -> str:
        """Render a table of contents section"""
        agents_info = [
            ("1", "Executive Summary", "📊", "Overview"),
            ("2", "IQVIA Market Intelligence", "📈", "iqvia"),
            ("3", "Clinical Trials Landscape", "🧬", "clinical"),
            ("4", "IP & Patent Analysis", "🛡️", "patent"),
            ("5", "Trade & Supply Chain", "🌍", "exim"),
            ("6", "Internal Knowledge", "📚", "internal_knowledge"),
            ("7", "External Intelligence", "🌐", "web_intelligence"),
        ]
        
        items_html = ""
        for num, label, icon, key in agents_info:
            if key == "Overview":
                status_class = "active"
                status_label = "INCLUDED"
            else:
                agent_data = data.get(key, {})
                has_data = bool(agent_data and (isinstance(agent_data, dict) and len(agent_data) > 0))
                status_class = "active" if has_data else "inactive"
                status_label = "DATA AVAILABLE" if has_data else "NO DATA"
            
            color = {
                "Overview": "var(--secondary-light)",
                "iqvia": "var(--iqvia)",
                "clinical": "var(--clinical)",
                "patent": "var(--patent)",
                "exim": "var(--exim)",
                "internal_knowledge": "var(--internal)",
                "web_intelligence": "var(--web)",
            }.get(key, "var(--accent)")
            ...
            <div class="toc-item">
                ...
                <span class="toc-badge" style="background: {'rgba(52,211,153,0.12)' if status_class == 'active' else 'rgba(107,114,128,0.12)'}; color: {'var(--success)' if status_class == 'active' else 'var(--text-faint)'}; border: 1px solid {'rgba(52,211,153,0.25)' if status_class == 'active' else 'rgba(107,114,128,0.2)'};">{status_label}</span>
            </div>
            '''
    ...
    def _render_agent_status_grid(self, data: Dict) -> str:
        """Render grid showing which agents contributed data"""
        agents = [
            ("iqvia", "📈", "IQVIA Insights"),
            ("clinical", "🧬", "Clinical Trials"),
            ("patent", "🛡️", "Patent Landscape"),
            ("exim", "🌍", "EXIM Trade"),
        ...
        for key, icon, name in agents:
            ...
            if has_data:
                active_count += 1
            css_key = key.replace("_knowledge", "").replace("_intelligence", "")
            
            cards_html += f'''
            <div class="agent-status-card {css_key}">
                <div class="agent-status-icon">{icon}</div>
                <div class="agent-status-name">{name}</div>
                <div class="agent-status-label {'active' if has_data else 'inactive'}">{'● Active' if has_data else '○ Inactive'}</div>
            </div>
            '''
        ...
        <div style="text-align: center; margin: 0 64px 48px; padding: 16px; background: var(--surface); border-radius: 12px; border: 1px solid var(--border);">
            <span style="font-family: 'DM Sans', sans-serif; font-size: 13px; color: var(--text-muted);">
                <span style="color: var(--accent); font-weight: 600;">{active_count}</span> of <span style="font-weight: 600;">6</span> intelligence agents contributed data to this report
            </span>
        </div>
        '''
...
<body>
    <div class="report-container">
        ...
        {self._render_table_of_contents(data)}
        {self._render_agent_status_grid(data)}
